    ...
    template: [
      {
        ...
        'kubernetes.pod.host_ip': '{{pod.ip}}',
        'kubernetes.pod.ip': '{{pod.ip}}',
        ...
        'kubernetes.pod.network.rx.bytes': ({ rxTotal }: CycleValues) => rxTotal,
        'kubernetes.pod.network.rx.errors': ({ rxTotal }: CycleValues) => rxTotal * 0.0002,
        'kubernetes.pod.network.tx.bytes': ({ txTotal }: CycleValues) => txTotal,
        'kubernetes.pod.network.tx.errors': ({ txTotal }: CycleValues) => txTotal * 0.0002,
